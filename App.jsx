...
function Detail({ item, onClose, onDelete, onStatusChange, onSave, statusCfg, statusOrder, editFields, extraAction }) {
  ...
  return (
    <div style={{ background: nude.white, border: `1px solid ${nude.border}`, borderRadius: 14, padding: 26, boxShadow: "0 2px 12px #c4a98812" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px 16px", marginBottom: 16 }}>
        {editFields.map(({ label, key, full, textarea }) => (
          <div key={key} style={{ gridColumn: full ? "span 2" : "span 1" }}>
            ...
            {textarea ? <textarea value={editData[key] || ""} rows={3} onChange={e => set(key)(e.target.value)} style={{ ...inputStyle, resize: "vertical" }} /> : <input value={editData[key] || ""} onChange={e => set(key)(e.target.value)} style={inputStyle} />}
          </div>
        ))}
      </div>
      ...
      {dirty && <button onClick={handleSave} disabled={saving} style={{ width: "100%", padding: "10px 0", background: `linear-gradient(135deg, ${nude.gold}, ${nude.gold2})`, border: "none", borderRadius: 10, color: "#fff", fontFamily: "'DM Mono', monospace", fontSize: 13, fontWeight: 700, cursor: "pointer", marginBottom: 12, opacity: saving ? 0.7 : 1 }}>{saving ? "Salvando…" : "SALVAR ALTERAÇÕES"}</button>}
...
export default function App() {
  ...
  const OS_FIELDS = [
    ...
    { label: "A/C (R$)", key: "ac" }, { label: "Valor de Custo (R$)", key: "nc" },
          ...
          <div style={{ display: "grid", gridTemplateColumns: selected ? "1fr 420px" : "1fr", minHeight: "calc(100vh - 200px)" }}>
            ...
            {selected && (
              <div style={{ padding: 20, overflowY: "auto", background: nude.bg }}>
                <Detail item={selected} onClose={() => setSelected(null)} onDelete={handleDelete}
                  onStatusChange={handleStatusChange} onSave={handleSaveEdit}
                  statusCfg={currentStatus} statusOrder={currentOrder} editFields={editFields}
                  extraAction={tab === "orc" && selected.status !== "Aprovado" && selected.status !== "Recusado" ? (
                    <button onClick={() => handleOrcToOS(selected)} style={{ width: "100%", padding: "10px 0", background: `linear-gradient(135deg, ${nude.gold}, ${nude.gold2})`, border: "none", borderRadius: 8, color: "#fff", fontFamily: "'DM Mono', monospace", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>✓ Converter em OS</button>
                  ) : null}
                />
              </div>
            )}
